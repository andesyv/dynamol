...
ImageDepthScaleRenderer::ImageDepthScaleRenderer(Viewer* viewer) : Renderer(viewer)
{
	...
	createShaderProgram("shadow", {
			{ GL_VERTEX_SHADER,"./res/sphere/sphere-vs.glsl" },
			{ GL_GEOMETRY_SHADER,"./res/sphere/sphere-gs-legacy.glsl" },
	...
	for (auto& d : std::filesystem::directory_iterator("./dat/environments"))
	{
		...
		std::unique_ptr<Texture> texture = SphereRenderer::loadTexture(environmentPath.string());
	...
	for (auto& d : std::filesystem::directory_iterator("./dat/materials"))
	{
		...
		std::unique_ptr<Texture> texture = SphereRenderer::loadTexture(materialPath.string());
	...
	for (auto& d : std::filesystem::directory_iterator("./dat/bumps"))
	{
		...
		std::unique_ptr<Texture> texture = SphereRenderer::loadTexture(bumpPath.string());
	...
	// Create two framebuffers for each LODs depth renderings:
	for (auto [fb, color, depth] : zip(m_LODDepthFramebuffer, m_LODDepthFramebuffer_Color, m_LODDepthFramebuffer_Depth)) {
		...
		color->image2D(0, GL_RGBA32F, m_framebufferSize, 0, GL_RGBA, GL_UNSIGNED_BYTE, nullptr);
...
void ImageDepthScaleRenderer::display()
{
	...
	auto iterationState = State::currentState();
	unsigned int LODIteration{0};
	for (; LODIteration < 2; ++LODIteration) {
		if (LODIteration != 0)
			iterationState->apply();
		const auto vertexCount = LODIteration == 0 ? m_sparseVertexCount : static_cast<GLsizei>(viewer()->scene()->protein()->atoms()[currentTimestep].size());

		// Vertex binding setup
		auto vertexBinding = m_vao->binding(0);
		vertexBinding->setAttribute(0);
		if (LODIteration == 0)
			vertexBinding->setBuffer(m_sparseAtomVertices.get(), 0, sizeof(vec4));
		else
		...
		// Shading
		//////////////////////////////////////////////////////////////////////////
		auto& LODfb = m_LODDepthFramebuffer[LODIteration];
		LODfb->bind();
		glClearDepth(1.f);
		...
		// Blit depth:
		m_surfaceFramebuffer->blit(GL_COLOR_ATTACHMENT0, { 0,0,viewer()->viewportSize().x, viewer()->viewportSize().y }, LODfb.get(), GL_COLOR_ATTACHMENT0, { 0,0,viewer()->viewportSize().x, viewer()->viewportSize().y }, GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT, GL_NEAREST);
